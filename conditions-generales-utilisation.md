# Conditions Générales d'Utilisation de BrocoQuests

**Date d'entrée en vigueur : 1 décembre 2025**

**Dernière mise à jour : 16 février 2026**

## 1. Informations légales

### 1.1 Éditeur de l'application

**BrocoQuests** est une application mobile éditée par :
- **Nom :** Jay Pierrick
- **Email :** brocoquests@gmail.com
- **Hébergement :** Railway (www.railway.app)

### 1.2 Acceptation des conditions

En créant un compte et en utilisant BrocoQuests, vous acceptez sans réserve les présentes Conditions Générales d'Utilisation (CGU). Si vous n'acceptez pas ces conditions, veuillez ne pas utiliser l'application.

## 2. Description du service

### 2.1 Présentation

BrocoQuests est une application mobile de productivité permettant aux utilisateurs de développer leurs capacités cérébrales.
Elle utilise un système de quête avec gamification pour motiver l’utilisateur à faire des quiz d’acuité cérébrale.

### 2.2 Accès au service

L'application est accessible :
- Sur iOS via l'App Store
- Sur Android via le Google Play Store

Une connexion Internet est requise pour utiliser l'application.

### 2.3 Évolution du service

Nous nous réservons le droit de faire évoluer, modifier ou suspendre, sans préavis, l'application pour des raisons de maintenance ou pour tout autre motif jugé nécessaire. Nous ne saurions être tenus responsables des conséquences éventuelles de ces modifications, suspensions ou interruptions.

## 3. Inscription et compte utilisateur

### 3.1 Conditions d'inscription

Pour utiliser BrocoQuests, vous devez :
- Avoir au moins 15 ans
- Fournir une adresse email valide
- Choisir un nom d'utilisateur et un mot de passe
- Accepter les présentes CGU et la Politique de Confidentialité

### 3.2 Exactitude des informations

Vous vous engagez à fournir des informations exactes, complètes et à jour lors de votre inscription. Vous êtes responsable de la mise à jour de vos informations personnelles.

### 3.3 Sécurité du compte

Vous êtes entièrement responsable de :
- La confidentialité de votre mot de passe
- Toutes les activités effectuées depuis votre compte
- Toute utilisation de votre compte, qu'elle soit autorisée ou non

En cas d'utilisation non autorisée de votre compte, vous devez nous en informer immédiatement à : brocoquests@gmail.com

### 3.4 Un seul compte par personne

Chaque utilisateur ne peut créer qu'un seul compte. La création de comptes multiples est interdite et peut entraîner la suspension de tous vos comptes.

## 4. Utilisation de l'application

### 4.1 Licence d'utilisation

Nous vous accordons une licence limitée, non exclusive, non transférable et révocable pour utiliser BrocoQuests à des fins personnelles et non commerciales, conformément aux présentes CGU.

### 4.2 Utilisations interdites

Vous vous engagez à NE PAS :

**Concernant le contenu :**
- Publier, transmettre ou partager du contenu illégal, offensant, diffamatoire, haineux, violent, pornographique ou contraire aux bonnes mœurs
- Usurper l'identité d'une autre personne ou entité
- Publier du contenu qui viole les droits de propriété intellectuelle de tiers
- Diffuser du spam, des chaînes de messages ou du contenu promotionnel non sollicité

**Concernant l'application :**
- Tenter de contourner les mesures de sécurité de l'application
- Utiliser des robots, scrapers ou autres moyens automatisés pour accéder à l'application
- Tenter de décompiler, désassembler ou effectuer de l'ingénierie inverse de l'application
- Perturber ou interférer avec le bon fonctionnement de l'application
- Exploiter des bugs ou vulnérabilités à des fins malveillantes (merci de nous les signaler)

**Concernant les autres utilisateurs :**
- Harceler, menacer ou intimider d'autres utilisateurs
- Collecter des informations sur d'autres utilisateurs sans leur consentement
- Utiliser l'application pour toute activité frauduleuse ou illégale

### 4.3 Conséquences du non-respect

En cas de non-respect de ces règles, nous nous réservons le droit de :
- Vous avertir
- Suspendre temporairement votre compte
- Supprimer définitivement votre compte
- Prendre toute action légale appropriée

## 5. Contenu utilisateur

### 5.1 Propriété du contenu

Vous conservez tous les droits sur le contenu que vous créez et partagez dans BrocoQuests (textes, tâches, objectifs, etc.).

### 5.2 Licence accordée à BrocoQuests

En publiant du contenu dans l'application, vous nous accordez une licence mondiale, non exclusive, libre de redevances pour :
- Stocker et afficher votre contenu dans le cadre du service
- Effectuer des sauvegardes techniques
- Améliorer le fonctionnement de l'application

Cette licence prend fin lorsque vous supprimez votre contenu ou votre compte.

### 5.3 Responsabilité du contenu

Vous êtes seul responsable du contenu que vous publiez. Nous ne contrôlons pas et ne sommes pas responsables du contenu publié par les utilisateurs.

Cependant, nous nous réservons le droit (sans obligation) de :
- Surveiller le contenu
- Retirer tout contenu qui viole ces CGU
- Signaler tout contenu illégal aux autorités compétentes

### 5.4 Signalement de contenu

Si vous identifiez du contenu inapproprié ou qui viole ces CGU, veuillez nous le signaler à : brocoquests@gmail.com

## 6. Propriété intellectuelle

### 6.1 Droits de BrocoQuests

L'application BrocoQuests, incluant sans limitation :
- Le code source
- Le design, l'interface utilisateur
- Le logo, les graphiques, les icônes
- Les textes, contenus éditoriaux
- Les fonctionnalités et algorithmes

sont la propriété exclusive de Jay Pierrick et sont protégés par les lois françaises et internationales sur la propriété intellectuelle et le droit d'auteur.

### 6.2 Marques

"BrocoQuests" et tous les logos associés sont des marques appartenant à Jay Pierrick. Toute utilisation non autorisée de ces marques est strictement interdite.

### 6.3 Interdiction de reproduction

Toute reproduction, représentation, modification, publication, adaptation de tout ou partie de l'application, quel que soit le moyen ou le procédé utilisé, est interdite, sauf autorisation écrite préalable de Jay Pierrick.

## 7. Données personnelles

Le traitement de vos données personnelles est régi par notre [Politique de Confidentialité](lien-vers-politique-confidentialite), qui fait partie intégrante des présentes CGU.

En utilisant BrocoQuests, vous acceptez également notre Politique de Confidentialité.

## 8. Disponibilité et maintenance

### 8.1 Disponibilité du service

Nous nous efforçons de maintenir l'application accessible 24h/24 et 7j/7. Cependant, nous ne garantissons pas une disponibilité ininterrompue et ne pourrons être tenus responsables de toute interruption de service.

### 8.2 Maintenance

L'application peut être temporairement indisponible pour maintenance. Nous nous efforcerons de vous prévenir à l'avance des maintenances planifiées.

## 9. Limitations de responsabilité

### 9.1 Utilisation à vos risques

Vous utilisez BrocoQuests à vos propres risques. L'application est fournie "en l'état" et "selon disponibilité", sans garantie d'aucune sorte, expresse ou implicite.

### 9.2 Exclusions de garantie

Nous ne garantissons pas que :
- L'application répondra à vos besoins spécifiques
- L'application sera ininterrompue, sécurisée ou exempte d'erreurs
- Les résultats obtenus seront exacts ou fiables
- Les défauts seront corrigés

### 9.3 Limitation de responsabilité

Dans toute la mesure permise par la loi applicable, Jay Pierrick ne saurait être tenu responsable de tout dommage direct, indirect, accessoire, spécial, consécutif ou exemplaire résultant de :
- L'utilisation ou l'impossibilité d'utiliser l'application
- La perte de données
- Les interruptions de service
- Les erreurs ou omissions dans le contenu
- Tout autre problème lié à l'utilisation de l'application

### 9.4 Exceptions

Les limitations de responsabilité ci-dessus ne s'appliquent pas en cas de :
- Faute lourde ou intentionnelle de notre part
- Dommages corporels
- Situations où la loi applicable ne permet pas l'exclusion ou la limitation de responsabilité

## 10. Suspension et résiliation

### 10.1 Suspension par nous

Nous pouvons suspendre ou résilier votre accès à BrocoQuests, immédiatement et sans préavis, si :
- Vous violez les présentes CGU
- Votre compte est utilisé de manière frauduleuse
- Cela est nécessaire pour des raisons de sécurité
- Nous sommes légalement tenus de le faire

### 10.2 Résiliation par vous

Vous pouvez à tout moment supprimer votre compte depuis les paramètres de l'application ou en nous contactant à : brocoquests@gmail.com

### 10.3 Conséquences de la résiliation

En cas de résiliation de votre compte :
- Vous perdrez l'accès à l'application
- Vos données seront supprimées conformément à notre Politique de Confidentialité (conservation de 30 jours pour sauvegarde, puis suppression définitive)
- Les dispositions des présentes CGU destinées à survivre à la résiliation resteront en vigueur

## 11. Modifications des CGU

### 11.1 Droit de modification

Nous nous réservons le droit de modifier les présentes CGU à tout moment. Les modifications entreront en vigueur dès leur publication dans l'application.

### 11.2 Notification des changements

En cas de modification substantielle, nous vous en informerons par :
- Une notification dans l'application
- Un email à l'adresse associée à votre compte

### 11.3 Acceptation des modifications

Votre utilisation continue de BrocoQuests après la publication des modifications vaut acceptation des nouvelles CGU. Si vous n'acceptez pas les modifications, vous devez cesser d'utiliser l'application et supprimer votre compte.

## 12. Dispositions générales

### 12.1 Intégralité de l'accord

Les présentes CGU, ainsi que la Politique de Confidentialité, constituent l'intégralité de l'accord entre vous et nous concernant l'utilisation de BrocoQuests.

### 12.2 Divisibilité

Si une disposition des présentes CGU est jugée invalide ou inapplicable, les autres dispositions resteront en vigueur.

### 12.3 Renonciation

Le fait de ne pas exercer un droit prévu par les présentes CGU ne constitue pas une renonciation à ce droit.

### 12.4 Cession

Vous ne pouvez pas céder ou transférer vos droits ou obligations en vertu des présentes CGU sans notre consentement écrit préalable. Nous pouvons librement céder nos droits et obligations.

### 12.5 Force majeure

Nous ne serons pas responsables de tout retard ou inexécution résultant de causes indépendantes de notre volonté raisonnable, y compris, sans limitation : catastrophes naturelles, guerres, actes de terrorisme, émeutes, pannes d'équipement, cyberattaques, ou défaillances des fournisseurs de services tiers.

## 13. Droit applicable et juridiction

### 13.1 Droit applicable

Les présentes CGU sont régies et interprétées conformément au droit français, sans tenir compte des principes de conflits de lois.

### 13.2 Règlement des litiges

**Médiation préalable :**
En cas de litige relatif à l'interprétation ou à l'exécution des présentes CGU, nous encourageons une résolution amiable. Vous pouvez nous contacter à : brocoquests@gmail.com

**Médiation de la consommation :**
Conformément à l'article L. 612-1 du Code de la consommation, vous avez le droit de recourir gratuitement à un médiateur de la consommation en vue de la résolution amiable d'un litige vous opposant à nous.

**Juridiction compétente :**
À défaut de résolution amiable, tout litige relatif aux présentes CGU sera soumis à la compétence exclusive des tribunaux français, conformément aux règles de compétence territoriale applicables.

## 14. Phase Beta

### 14.1 Nature du service

BrocoQuests est actuellement en phase **Beta** (test). Cela signifie que :
- L'application est en cours de développement et peut contenir des bugs
- Des fonctionnalités peuvent être ajoutées, modifiées ou supprimées sans préavis
- Le service peut être interrompu temporairement pour des améliorations
- Vos retours et suggestions sont essentiels et bienvenus

### 14.2 Pas de garantie de continuité

Durant la phase Beta, nous ne garantissons pas la continuité du service. Le service peut être arrêté définitivement si le projet n'aboutit pas.

### 14.3 Gratuit pendant la Beta

L'application est actuellement gratuite. Nous nous réservons le droit d'introduire des fonctionnalités payantes ou un modèle d'abonnement à l'avenir. Les utilisateurs Beta seront informés à l'avance de tout changement de tarification.

## 15. Contact

Pour toute question concernant ces Conditions Générales d'Utilisation :

**Email :** brocoquests@gmail.com

**Support technique :** Même adresse email

**Retours et suggestions :** Nous sommes à l'écoute de vos retours pour améliorer BrocoQuests !

---

**Merci d'utiliser BrocoQuests et de contribuer à son amélioration !**

**Version Beta - Novembre 2024**
